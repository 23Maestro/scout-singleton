...
export async function updateAsanaFields(taskGid: string, patch: Partial<VideoTask> & { playerId?: string; athleteName?: string }) {
  ...
  const fields: Array<{ gid: string; name: string }> = (detail.data?.data?.custom_fields || []).map((f: any) => ({ gid: f.gid, name: f.name }));
