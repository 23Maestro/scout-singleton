...
function getStatusIcon(status: TaskStatus): Icon {
  switch (status) {
    case "Revisions": return Icon.ArrowClockwise;
    case "HUDL": return Icon.CircleFilled;
    case "Dropbox": return Icon.Folder;
    case "External Links": return Icon.Link;
...
function getStatusColor(status: TaskStatus): Color {
  switch (status) {
    case "Revisions": return Color.Purple;
    case "HUDL": return Color.Red;
    case "Dropbox": return Color.Blue;
    case "External Links": return Color.Green;
...
function getStageIcon(stage: TaskStage): Icon {
  switch (stage) {
    case "On Hold": return Icon.Pause;
    case "Awaiting Client": return Icon.Clock;
    case "In Queue": return Icon.Play;
    ...
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          ...
          {task.sport && (
            <Detail.Metadata.Label title="Sport" text={task.sport} />
          )}
          {task.gradYear && (
            <Detail.Metadata.Label title="Grad Year" text={task.gradYear} />
          )}
          {task.positions && (
            <Detail.Metadata.Label title="Positions" text={task.positions} />
          )}
          {task.city && task.state && (
            <Detail.Metadata.Label title="Location" text={`${task.city}, ${task.state}`} />
          )}
          {task.highSchool && (
            <Detail.Metadata.Label title="High School" text={task.highSchool} />
          )}
          {task.playerId && (
            <Detail.Metadata.Label title="Player ID" text={task.playerId} />
          )}
          {task.profileUrl && (
            <Detail.Metadata.Link title="Profile URL" target={task.profileUrl} text="View Profile" />
          )}
          {task.paymentStatus && (
          ...
          <Detail.Metadata.Link title="Asana Link" target={task.permalinkUrl} text="Open in Asana" />
        </Detail.Metadata>
      }
      actions={
        <ActionPanel>
      ...
      searchBarAccessory={
        ...
        >
          ...
          <List.Dropdown.Section title="Video Status">
            <List.Dropdown.Item title="Revisions" value="Revisions" icon={{ source: Icon.ArrowClockwise, tintColor: Color.Purple }} />
            <List.Dropdown.Item title="HUDL" value="HUDL" icon={{ source: Icon.CircleFilled, tintColor: Color.Red }} />
            <List.Dropdown.Item title="Dropbox" value="Dropbox" icon={{ source: Icon.Folder, tintColor: Color.Blue }} />
            <List.Dropdown.Item title="External Links" value="External Links" icon={{ source: Icon.Link, tintColor: Color.Green }} />
            <List.Dropdown.Item title="Not Approved" value="Not Approved" icon={{ source: Icon.XMarkCircle, tintColor: Color.Orange }} />
          </List.Dropdown.Section>
        </List.Dropdown>
      }
    >
      {(["In Queue", "Awaiting Client", "On Hold", "Done", "Unknown"] as TaskStage[]).map((stage) => {
          ...
          <List.Section key={stage} title={stage} subtitle={`${stageTasks.length} tasks`}>
            {stageTasks.map((task) => (
              <List.Item
                key={task.id}
                title={task.taskName}
                subtitle={[task.athleteName, task.sport, task.gradYear].filter(Boolean).join(" • ")}
                ...
                accessories={[
                  ...
                  task.positions ? { text: task.positions, icon: Icon.Person, tooltip: `Positions: ${task.positions}` } : null,
                  task.dueOn
                    ? {
                        text: formatDistanceToNow(new Date(task.dueOn), { addSuffix: true }),
                        icon: Icon.Calendar,
                      ...
                      <Action
                        ...
                        onAction={async () => {
                          ...
                          for (const t of doneTasks) {
                            try {
                              const beforeCount = [t.city, t.state, t.highSchool, t.positions, t.sport, t.gradYear, t.paymentStatus, t.playerId, t.athleteName].filter(Boolean).length;
