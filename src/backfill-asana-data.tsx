...
export default function BackfillAsanaData() {
  ...
  const loadOldTasks = async () => {
    try {
      ...
      const workspace =
        workspaces.find(
          (ws: { name: string; gid: string }) => ws.name === 'National Prospect ID',
        ) || workspaces[0];
      ...
      const analyzedTasks = oldTasks
        .map(
          (task: { gid: string; name: string; created_at: string; custom_fields: unknown[] }) => {
        ...
        .filter(
          (task: { missingFields: string[]; playerName: string }) =>
            task.missingFields.length > 0 && task.playerName,
        );
      ...
      task.missingFields.forEach((fieldName) => {
        ...
        switch (fieldName) {
          ...
          case 'Sport': {
            ...
            const sportKey = playerData.sport
              ?.replace(/[\u{1F3C8}\u{1F3C0}\u{26BE}\u{1F94E}]/gu, '')
              .trim();
