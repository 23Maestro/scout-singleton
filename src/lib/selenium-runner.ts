...
export async function runSeleniumPlayerSearch(
  athleteName: string,
): Promise<SeleniumSearchResult[]> {
    ...
    exec(
      ...
      (error, stdout, stderr) => {
        ...
        try {
          ...
          const mappedResults: SeleniumSearchResult[] = results.map(
...
export async function runSeleniumInboxExtraction(): Promise<InboxThread[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const scriptPath =
      '/Users/singleton23/Raycast/scout-singleton/scripts/extract_inbox_data_fixed.py';
