...
export async function runSeleniumInboxExtraction(): Promise<InboxThread[]> {
  ...
  return new Promise((resolve, reject) => {
    const scriptPath =
      '/Users/singleton23/Raycast/scout-singleton/scripts/extract_inbox_data_minimal.py';
    ...
    exec(
      command,
      {
        env: {
          ...process.env,
          PATH:
            '/Library/Frameworks/Python.framework/Versions/3.13/bin:/opt/homebrew/bin:' +
            process.env.PATH,
      ...
      (error, stdout, stderr) => {
        ...
        try {
          ...
          if (output.startsWith('{')) {
            ...
            console.log(
              `✅ Minimal API success: ${result.length} threads, ${parsed.unread} unread`,
            );
          } else {
