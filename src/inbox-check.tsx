...
function extractJsonFromMCPResponse(responseText: string): any {
  try {
    ...
    if (endIndex === -1) {
      console.error(
        'Could not find matching closing bracket/brace. JSON text length:',
        jsonText.length,
      );
...
function isMessagePartOfOngoingThread(allMessages: any[], currentMessage: any): boolean {
  ...
  const sameEmailMessages = allMessages.filter((msg) => msg.email === currentMessage.email);
...
function AssignmentModal({
  ...
  return (
    ...
    >
      ...
      <Form.Separator />
      <Form.Dropdown id="assignee" title="Assigned Owner" value={assignee} onChange={setAssignee}>
...
🔗 Player Profile: https://nationalprospectid.com/player/${message.playerId}`,
    ...
    if (error && typeof error === 'object' && 'response' in error) {
      ...
      throw new Error(
        `Asana API Error: ${axiosError.response?.data?.errors?.[0]?.message || axiosError.message}`,
      );
    }
    throw error;
  }
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const transformedMessages: InboxMessage[] = allThreads.map((thread: any, index: number) => {
          ...
          console.log(
            `🔍 ${playerName}: ${isAssigned ? 'ASSIGNED (per NPID API)' : 'UNASSIGNED (per NPID API)'}`,
          );
  ...
  return (
    <List
      ...
      searchBarAccessory={
        ...
        >
          <List.Dropdown.Item
            title="🆕 Unassigned (New Inquiries)"
            value="unassigned"
            icon={Icon.Envelope}
          />
          <List.Dropdown.Item
            title="🔄 Assigned (Ongoing Threads)"
            value="assigned"
            icon={Icon.Check}
          />
        </List.Dropdown>
